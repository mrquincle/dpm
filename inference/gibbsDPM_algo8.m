...
function c_st = gibbsDPM_algo8(y, hyperG0, alpha, niter, doPlot)
    ...
    for i=2:niter
        ...
        for k=1:n
            ...
            if doPlot==1
                L = get_likelihoods(m, y, hyperG0.prior, U_R, c);
                plot_assignments(y, hyperG0, U_R, m, c, k, i, cmap, L);
            end
        end
...
function [dR] = update_c(m, alpha, z, hyperG0, U_R, c)
    ...
    for tt=1:100
        ...
        for i=1:length(cind)
            ...
            nprop = combine_likelihoods(nprop_v);
            % likelihood that observations are generated by the existing pdf
            ncurr = combine_likelihoods(likelihoods(hyperG0.prior, z(:,cust), dR(j) ));
            if (ncurr)
                alpha2=nprop/ncurr;
                u=rand(1);
...
function L = get_likelihoods(m, z, likelihood_type, U_R, c)
    ...
    for i = 1:length(cind)
        ...
        L(i) = combine_likelihoods(likelihoods(likelihood_type, z_set, U_R(j) ));
    end
end
...
%        disp("Set of observations:");
 %       disp(z_set);
    ...
    % display the top K clusters w.r.t. likelihood
    K = 2;
    [Lmax, Li ] = maximum(L, K);
    for i = 1:length(Li)
        cluster=Li(i);
        ...
        printf("Cluster %i and table %i has likelihood: %f\n", cluster, table, likelihood);
        disp("Observations at this table");
        disp(observations_at_this_table);
        disp("Parameters of this table");
        disp(parameters);
        disp("Likelihoods at this table");
        disp(likelihoods_at_this_table);
    end
end

function P = combine_likelihoods(likelihoods)
    % P = prod(likelihoods);
    P = mean(likelihoods); 
end
